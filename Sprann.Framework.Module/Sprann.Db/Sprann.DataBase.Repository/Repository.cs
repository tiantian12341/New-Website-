...
namespace Sprann.DataBase.Repository
{
    ...
    public class Repository : IRepository, IDisposable
    {
        ...
        /// <summary>
        /// 创建数据库执行的相关命令,并附加到连接对象DbConnection。
        /// <para>（2019-1-4 Ridge W.)</para>
        /// </summary>
        /// <returns></returns>
        public DbCommand CreateDbCommand()
        {
            DbCommand myCmd = db.CreateDbCommand();
            myCmd.CommandTimeout = 500;
            myCmd.CommandType = CommandType.Text;
            myCmd.Connection = getDbConnection();
            return myCmd;
        }
        ...
        {
            return db.FindTableByProc(procName, dbParameter);
        }
        ...
        {
            return db.FindDataSetByProc(procName, dbParameter);
        }
        ...
        public IEnumerable<T> FindList<T>(string strSql, Pagination pagination) where T : class
        {
            ...
            {
                var data = db.FindList<T>(strSql, pagination.sidx, pagination.sord.ToLower() == "asc" ? true : false, pagination.rows, pagination.page, out total).OrderBy(x => "ID");
                pagination.records = total;
                return data;
            }
        }
        ...
        /// <param name="pagination">分页数据</param>
        /// <returns></returns>
        public DataSet FindDataSet(string strSql, object dbParameter, Pagination pagination)
        {
            int total = pagination.records;
            DataSet data;
            if (dbWhere != null)
            {
                ...
                DynamicParameters dynamicParameters = SqlHelper.FieldValueParamToParameter(dbWhere.dbParameters);
                dynamicParameters.AddDynamicParams(dbParameter);
                data = db.FindDataSet(strSql, dynamicParameters, pagination.sidx, pagination.sord.ToLower() == "asc" ? true : false, pagination.rows, pagination.page, out total);
            }
            else
            {
                data = db.FindDataSet(strSql, dbParameter, pagination.sidx, pagination.sord.ToLower() == "asc" ? true : false, pagination.rows, pagination.page, out total);
            }
            pagination.records = total;
            return data;
        }
