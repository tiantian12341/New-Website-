...
namespace Sprann.DataBase.SqlServer
{
    /// <summary>
    /// 日 期：2017.03.04
    /// 描 述：数据库操作类
    ...
    public class Database : IDatabase
    {
        ...
        public int ExecuteBySql(string strSql)
        {
            try
            {
                return dbcontext.Database.Connection.Execute(strSql, null, dbTransaction, 500);
            }
            catch (Exception)
            {
        ...
        public int ExecuteBySql(string strSql, object dbParameter)
        {
            try
            {
                return dbcontext.Database.Connection.Execute(strSql, dbParameter, dbTransaction, 500);
            }
            catch (Exception)
            {
        ...
        public int ExecuteByProc(string procName)
        {
            try
            {
                return dbcontext.Database.Connection.Execute(procName, null, dbTransaction, 500, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public int ExecuteByProc(string procName, object dbParameter)
        {
            try
            {
                return dbcontext.Database.Connection.Execute(procName, dbParameter, dbTransaction, 500, CommandType.StoredProcedure);
            }
            catch (Exception ex)
            {
        ...
        public T ExecuteByProc<T>(string procName) where T : class
        {
            try
            {
                return dbcontext.Database.Connection.ExecuteScalar<T>(procName, null, dbTransaction, 500, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public T ExecuteByProc<T>(string procName, object dbParameter) where T : class
        {
            try
            {
                return dbcontext.Database.Connection.ExecuteScalar<T>(procName, dbParameter, dbTransaction, 500, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public DataTable FindTableByProc(string procName, object dbParameter)
        {
            try
            {
                var IDataReader = dbcontext.Database.Connection.ExecuteReader(procName, dbParameter, dbTransaction, 500, CommandType.StoredProcedure);
        ...
        /// <param name="dbParameter">参数</param>
        /// <returns></returns>
        public DataSet FindDataSetByProc(string procName, object dbParameter)
        {
            try
            {
                var IDataReader = dbcontext.Database.Connection.ExecuteReader(procName, dbParameter, dbTransaction, 500, CommandType.StoredProcedure);
                DataSet ds = new XDataSet();
                ds.Load(IDataReader, LoadOption.OverwriteChanges, null, new DataTable[] { });
                return ds;
            }
            catch (Exception)
            {
        ...
        public int Count<T>(Expression<Func<T, bool>> condition) where T : class, new()
        {
            try
            {
                return dbcontext.Set<T>().Where(condition).Count();
            }
            catch (Exception)
            {
        ...
        public DataTable FindTable(string strSql)
        {
            try
            {
                var IDataReader = dbcontext.Database.Connection.ExecuteReader(strSql, null, dbTransaction,500);
                return SqlHelper.IDataReaderToDataTable(IDataReader);
            }
            catch (Exception)
            {
        ...
        public DataTable FindTable(string strSql, object dbParameter)
        {
            try
            {
                var IDataReader = dbcontext.Database.Connection.ExecuteReader(strSql, dbParameter, dbTransaction, 500);
                return SqlHelper.IDataReaderToDataTable(IDataReader);
            }
            catch (Exception)
            {
        ...
        public DataTable FindTable(string strSql, object dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            ...
            try
            {
                ...
                //做sql字符串处理，将sql字段去除，替换成Count(1) 来查询数据条数
                int Indexof1 = strSql.ToLower().IndexOf("select");
                int Indexof2 = strSql.ToLower().IndexOf("from");

                var s = strSql.ToLower().Remove(Indexof1, Indexof2 - 1);
                

                string str=  strSql.ToLower().Remove(Indexof1, Indexof2-1).Insert(Indexof1, "select count(1) ");
                //string str = strSql.ToLower().Remove(Indexof2, 1).Insert(Indexof2, "select count(1) from");
                total = Convert.ToInt32(dbcontext.Database.Connection.ExecuteScalar(str, dbParameter));
                var IDataReader = dbcontext.Database.Connection.ExecuteReader(sb.ToString(), dbParameter, dbTransaction, 500, CommandType.Text);
                return SqlHelper.IDataReaderToDataTable(IDataReader);
            }
            catch (Exception ex)
            {
        ...
        /// <param name="total">总共数据条数</param>
        /// <returns></returns>
        public DataSet FindDataSet(string strSql, object dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(SqlHelper.SqlPageSql(strSql, orderField, isAsc, pageSize, pageIndex));
            try
