...
namespace Sprann.Util
{
    ...
    public class Config
    {
        ...
        /// 获取配置文件的基础路径
        /// </summary>
        /// <param name="key">AppSetting键值</param>
        /// <returns></returns>
        public static string GetBaseDirectory(string key)
        {
            object appVal = ConfigurationManager.AppSettings[key];
            if (appVal == null)
            {
                return GetAppDomainBaseDirectory();
            }
            else
            {
                string sVal = appVal.ToString();
                if (sVal.IndexOf(System.IO.Path.VolumeSeparatorChar) != -1)
                {
                    return sVal.Trim();
                }
                else
                {
                    ...
                    {
                        return System.Web.HttpContext.Current.Server.MapPath(sVal);
                    }
                    else
                    {
                        return System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, sVal);
                    }
                }
            }
        }
