...
namespace Sprann.Util
{
    ...
    public class ExcelHelper
    {
        ...
        public static MemoryStream ExportMemoryStream(DataTable dtSource, ExcelConfig excelConfig)
        {
            ...
            var columns = excelConfig.ColumnEntity.Select(p => p.Column.ToLower()).ToArray();
            var columnsStrings = string.Join(",", columns);

            for (int p = 0; p < dtSource.Columns.Count; p++)
            {
                if (!columnsStrings.Contains(dtSource.Columns[p].ColumnName.ToLower()))
                {
                    dtSource.Columns.Remove(dtSource.Columns[p].ColumnName);
                    p--;
                }
            }
            ...
            #region 右击文件 属性信息
            {
                ...
                si.ApplicationName = "思普瑞云"; //填加xls文件创建程序信息
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                foreach (DataColumn column in dtSource.Columns)
                {
                    ...
                    if (column.ColumnName== "±2s")
                    {
                        if (double.Parse(drValue)>0)
                        {
