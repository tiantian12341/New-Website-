...
namespace Sprann.Application.Form
{
    /// <summary>
    ...
    public class FormSchemeBLL : FormSchemeIBLL
    {
        ...
        private string GetQuerySql(FormSchemeModel formSchemeModel, JObject queryParam)
        {
            try
            {
                ...
                if (queryParam != null && !queryParam["spdateField"].IsEmpty())
                {
                    queryParamTemp.Add("spbegin", queryParam["spbegin"].ToDate());
                    queryParamTemp.Add("spend", queryParam["spend"].ToDate());
                    querySql += " AND (" + formSchemeModel.mainTableName + "tt." + queryParam["spdateField"].ToString() + " >=@spbegin AND " + formSchemeModel.mainTableName + "tt." + queryParam["spdateField"].ToString() + " <=@spend ) ";
                }
