...
namespace Sprann.Application.Excel
{
    ...
    public class ExcelImportBLL : ExcelImportIBLL
    {
            ...
            try
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (entity != null && list.Count > 0)
                    {
                        ...
                        string updateSql = " UPDATE " + entity.F_DbTable + " SET ";
                        string updateWhere = "";
                        string sqlValue = "(";
                        bool isfirt = true, updateWhereFirst = true, updateSqlFirst = true;

                        foreach (var field in list)
                        {
                            #region 处理更新SQL
                            if (field.F_OnlyOne.HasValue && field.F_OnlyOne.Value == 1)
                            {
                                if (!updateWhereFirst)
                                    updateWhere += " AND ";

                                updateWhere += " " + field.F_Name + " = @" + field.F_Name;
                                updateWhereFirst = false;
                            }
                            else
                            {
                                if (!updateSqlFirst)
                                    updateSql += ", ";
                                updateSql += " " + field.F_Name + " = @" + field.F_Name;
                        ...
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            try
                            {
                                ...
                                bool doUpdate = false;
                                //var idRowList = list.Where(c => c.F_OnlyOne.HasValue && c.F_OnlyOne.Value == 1).ToList();
                                if (!string.IsNullOrEmpty(updateWhere))
                                {
                                    doUpdate = ExistsTableRecord(entity.F_DbId, entity.F_DbTable, updateWhere, dp);
                                }
                                ...
                                {
                                    databaseLinkIBLL.ExecuteBySql(entity.F_DbId, updateSql, dp);
                                }
                                snum++;
                            }
                            catch (Exception ex)
                            {
                                ...
                                {
                                    dr["导入错误"] = ex.OriginalException().Message + "【之后数据未被导入】";
                                    failDt.Rows.Add(dr.ItemArray);
                                    break;
                                }
                                else
                                {
                                    dr["导入错误"] = ex.OriginalException().Message;
                                    failDt.Rows.Add(dr.ItemArray);
                                }
                            }
                        }
        ...
        private bool ExistsTableRecord(string dbId, string tableName, string updateWhere, DynamicParameters dp)
        {
            string sqlCheck = "Select top 1 * from " + tableName + " WHERE " + updateWhere;
            var d = databaseLinkIBLL.FindTable(dbId, sqlCheck, dp);
