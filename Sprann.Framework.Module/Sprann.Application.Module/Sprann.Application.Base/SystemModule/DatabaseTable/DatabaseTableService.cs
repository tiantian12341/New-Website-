...
namespace Sprann.Application.Base.SystemModule
{
    ...
    public class DatabaseTableService : RepositoryFactory
    {
        ...
        /// <summary>
        /// 执行存储过程并返回一个数据表
        /// <para>2018-12-28 Ridge W.</para>
        /// </summary>
        /// <param name="databaseLinkEntity"></param>
        /// <param name="spName"></param>
        /// <param name="paramList"></param>
        /// <returns></returns>
        public DataTable ExecuteSpTableResult(DatabaseLinkEntity databaseLinkEntity, string spName, List<SpParameterModel> paramList)
        {
            try
            {
                IRepository repository = this.BaseRepository(databaseLinkEntity.F_DbConnection, databaseLinkEntity.F_DbType);
                if (paramList == null || paramList.Count == 0)
                {
                    return repository.FindTableByProc(spName, null);
                }
                else
                {
                    SpParameterModel paramModel;
                    dynamic myParams = new ExpandoObject();
                    for (int i = 0, j = paramList.Count; i < j; i++)
                    {
                        paramModel = paramList[i];
                        (myParams as ICollection<KeyValuePair<string, object>>).Add(new KeyValuePair<string, object>(paramModel.name,
                            paramModel.GetParameterValue()));
                    }
                    return repository.FindTableByProc(spName, myParams);
                }
            }
            catch (Exception ex)
            {
