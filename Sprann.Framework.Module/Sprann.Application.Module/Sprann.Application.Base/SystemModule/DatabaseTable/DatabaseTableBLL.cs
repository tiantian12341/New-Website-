...
namespace Sprann.Application.Base.SystemModule
{
    ...
    public class DatabaseTableBLL : DatabaseTableIBLL
    {
        ...
        /// <returns></returns>
        public DataTable ExecuteSpTableResult(string databaseLinkId, string spName, List<SpParameterModel> paramList)
        {
            try
            {
                DatabaseLinkEntity databaseLinkEntity = databaseLinkIBLL.GetEntity(databaseLinkId);
                return databaseTableService.ExecuteSpTableResult(databaseLinkEntity, spName, paramList);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
