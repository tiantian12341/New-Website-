    ...
    public class DatabaseLinkBLL : DatabaseLinkIBLL
    {
        ...
        public List<DatabaseLinkEntity> GetList()
        {
            try
            {
                ...
                if (list == null || list.Any(l => l.F_DbConnection == "******")) //修复缓存数据库连接字符串（2018-12-25, Ridge)
                {
        ...
        public DatabaseLinkEntity GetEntity(string databaseLinkId)
        {
            ...
            {
                List<DatabaseLinkEntity> list = GetList();
                //GUID传值调整 2018-12-28 Ridge W.
                if (databaseLinkId.Length == "f2d587de-43e5-4310-b968-4544f4961a39".Length)
                {
                    return list.Find(t => t.F_DatabaseLinkId.Equals(databaseLinkId));
                }
                else
                {
                    return list.Find(t => t.F_DBName.Equals(databaseLinkId));
                }
            }
            catch (Exception ex)
            {
