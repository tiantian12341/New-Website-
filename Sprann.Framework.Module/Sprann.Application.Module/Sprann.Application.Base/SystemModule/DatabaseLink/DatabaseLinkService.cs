    ...
    public class DatabaseLinkService : RepositoryFactory
    {
        ...
        public IEnumerable<DatabaseLinkEntity> GetList()
        {
            try
            {
                ...
                IEnumerable<DatabaseLinkEntity> dbList = BaseRepository().FindList<DatabaseLinkEntity>(strSql.ToString());
                AppDomainDatabaseLinkObject linkObject = AppDomainDatabaseLinkObject.RestoreInAppDomain();
                if (linkObject== null || linkObject.LastSyncDatetime.HasValue && linkObject.LastSyncDatetime.Value < DateTime.Now)
                    SyncDatabaseLinkInAppDomain(dbList);
        ...
        /// 保存所有数据库连接信息到应用程序域
        /// </summary>
        /// <param name="dbList">需要保存的数据库连接信息</param>
        public static void SyncDatabaseLinkInAppDomain(IEnumerable<DatabaseLinkEntity> dbList = null)
        {
            if (dbList == null)
                dbList = (new DatabaseLinkService()).GetList();

            List<AppDomainDatabaseLink> myLinks = new List<AppDomainDatabaseLink>();
            foreach (var item in dbList)
            {
                myLinks.Add(new AppDomainDatabaseLink { LinkName = item.F_DBName, ProviderName = item.F_DbType, ConnectionString = item.F_DbConnection });
            }

            AppDomainDatabaseLinkObject linkObject = new AppDomainDatabaseLinkObject();
            linkObject.LastSyncDatetime = new DateTimeOffset(DateTime.Now);
            linkObject.Links = myLinks.ToArray();
            linkObject.StoreInAppDomain();
        }
