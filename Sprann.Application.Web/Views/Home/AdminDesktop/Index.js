...
(function ($, sp) {
    ...
    function loadDesktop() {
        ...
        if (loginInfo) {
            ...
            sp.httpAsync('GET', top.$.rootUrl + '/LR_FormModule/FormRelation/GetPreviewList', par, function (data) {
                if (data) {
                    $("#sp_target").html("");
                    var powerData = sortAndPower(loginInfo, data);
                    $.each(powerData, function (index, itemobj) {
                        //往桌面添加统计小功能
                        var width = parseInt($("#sp_target").width() / powerData.length);
                        width = width < 220 ? 220 : width;
                        var htmlStr = "";
                        htmlStr += '<div class="sp-item-20" style="width: ' + width + 'px;">';
                        ...
                        htmlStr += '</div></div>';
                        $("#sp_target").append(htmlStr);
                        //绑定数值
                        //sp.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/Desktop/GetSqlData', { databaseLinkId: itemobj.f_databaselinkid0, sql: itemobj.f_sql0, context: itemobj.f_targetid0 }, function (data) {
                        //    $("#" + this.context + "number").html(data[0].value);
                        //});
                        var res = sp.httpGet(top.$.rootUrl + '/LR_SystemModule/Desktop/GetSqlData', { databaseLinkId: itemobj.f_databaselinkid0, sql: itemobj.f_sql0 });
                        if (res) {
                            $("#" + itemobj.f_targetid0 + "number").html(res.data[0].value);
                        }
                        //绑定跳转
                        $('#' + itemobj.f_targetid0 + "more").on('click', itemobj, function (event) {
                            sp.frameTab.open({ F_ModuleId: event.data.f_targetid0, F_Icon: event.data.f_icon0, F_FullName: event.data.f_name0, F_UrlAddress: event.data.f_url0 });
                        });
                    });
                    //添加滚动条
                    $("#sp_target").spscroll();
                    if ($("#sp_target").width() < 1100) {
                        $("#sp_target_box").css("width", '1100px');
                    }
                    isScroll++;
                }
            });
            ...
            sp.httpAsync('GET', top.$.rootUrl + '/LR_FormModule/FormRelation/GetPreviewList', par2, function (data) {
                if (data) {
                    var powerData = sortAndPower(loginInfo, data);
                    $.each(powerData, function (index, itemobj) {
                        var htmlStr = "";
                        ...
                        if (itemobj.f_getdatatype0 == "0") {//Sql语句获取方式
                            res = sp.httpGet(top.$.rootUrl + itemobj.f_interface0, { databaseLinkId: itemobj.f_databaselinkid0, sql: itemobj.f_sql0 });
                            if (res) {
                                for (var j = 0; j < res.data.length; j++) {
                                    res.data[j].f_itemurl = itemobj.f_itemurl0;
                                    htmlStr += '        <div class="sp-msg-line">';
                            ...
                            res = sp.httpGet(top.$.rootUrl + itemobj.f_interface0, param);
                            if (res) {
                        ...
                        $('#' + itemobj.f_mlistid0 + "more").on('click', itemobj, function (event) {
                            sp.frameTab.open({ F_ModuleId: event.data.f_mlistid0, F_Icon: event.data.f_icon0, F_FullName: event.data.f_name0, F_UrlAddress: event.data.f_url0 });
                        });
                        //绑定行跳转
                        if (itemobj.f_getdatatype0 == "0") {//Sql语句获取方式
                            if (res) {
                                for (var j = 0; j < res.data.length; j++) {
                                    $('#' + res.data[j].f_id + "item").on('click', res.data[j], function (event) {
                                        ...
                                        sp.frameTab.open({ F_ModuleId: event.data.f_id, F_Icon: '', F_FullName: event.data.f_title, F_UrlAddress: openurl });
                                    });
                                }
                            }
                        } else if (itemobj.f_getdatatype0 == "1") {//接口获取方式
            ...
            sp.httpAsync('GET', top.$.rootUrl + '/LR_FormModule/FormRelation/GetPreviewList', par3, function (data) {
                if (data) {
                    var powerData = sortAndPower(loginInfo, data);
                    $.each(powerData, function (index, itemobj) {
                        var htmlStr = "";
                        ...
                        htmlStr += "        <div id=\"" + itemobj.f_chartid0 + "chart\" class=\"sp-chart-container\"></div>";
                        htmlStr += "    </div>";
                        htmlStr += "</div>";
    ...
    function isspscroll() {
        if (isScroll == 3) {//当isScroll==3时，实例化滚动条
            $(".sp-desktop-panel").spscroll();
        } else {//如果当前没有异步加载完页面，准备下次执行
            setTimeout(function () { isspscroll(); }, 100);
        }
    }
    isspscroll();
    ...
    window.onresize = function (e) {
        ...
        if ($("#sp_target").width() != 0) {
            //动态调整指标块宽度
            $("#sp_target").find(".sp-item-20").each(function () {
                var width = parseInt($("#sp_target").width() / $("#sp_target").find(".sp-item-20").length);
                width = width < 220 ? 220 : width;
                $(this).css("width", width + 'px');
            });
            //根据页面宽度调整信息列表列是否展示
            if ($("#sp_target").width() < 700) {
                $(".sp-msg-line").find("label").each(function () {
