...
var bootstrap = function ($, sp) {
    ...
    function setRowOp() {
        ...
        $('#parentId').spselectRefresh({
            data: res
        });

        //$('#colop').spSetFormData(currentRow);
        $('#name').val(currentRow.name);
        $('#parentId').spselectSet(parentId);
        $('#field').spselectSet(currentRow.field);
        $('#align').spselectSet(currentRow.align);
        $('#width').val(currentRow.width);
        $('#type').spselectSet(currentRow.type);
        ...
        switch (currentRow.type) {
            ...
            case 'checkbox':
            case 'select':
                $('#dataSource').spselectSet(currentRow.dataSource);
                $('#dfvalue').spselectSet(currentRow.dfvalue);
                if (currentRow.dataSource === '0') {
                    $('#dataItemId').spselectSet(currentRow.itemCode);
                }
                else {
                    $('#dataSourceId').spselectSet(currentRow.dataSourceId);
                    $('#showField').spselectSet(currentRow.showField);
                    $('#saveField').spselectSet(currentRow.saveField);
                }
                break;
            case 'layer':
                ...
                break;
            case 'datetime':
                $('#datetime').spselectSet(currentRow.datetime);
                break;
        }
    ...
    var page = {
        ...
        bind: function () {
                ...
                select: function (item) {
                    ...
                    switch (item.id) {
                        ...
                        case 'select':
                            ...
                            $('#dataSource').spselectSet(currentRow.dataSource);
                            if (currentRow.dataSource === '0') {
                                $('#dataItemId').spselectSet(currentRow.itemCode || currentRow.dataItemCode);
                            }
                            else {
                                $('#dataSourceId').spselectSet(currentRow.dataSourceId);
                                $('#showField').spselectSet(currentRow.showField);
            ...
            $('#dataItemId').spselect({
                ...
                select: function (item) {
                    if (item) {
                        ...
                        if (currentRow.type === 'radio' || currentRow.type === 'checkbox' || currentRow.type === 'select') {
                            currentRow.dfvalue = '';
                            $('#dfvalue').spselectRefresh({ url: '', data: [] });
                        }
                    }
                }
            });
                    ...
                    if (item) {
                        ...
                        if (currentRow.type === 'radio' || currentRow.type === 'checkbox' || currentRow.type === 'select') {
                            ...
                            sp.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DataSource/GetDataColName', { code: item.F_Code }, function (data) {
                                ...
                                $('#showField').spselectRefresh({
                                    data: fieldData,
                                    placeholder: false
                                });
                                $('#showField').spselectSet(currentRow.showField || fieldData[0].value);
                                $('#saveField').spselectRefresh({
                                    data: fieldData,
                                    placeholder: false
                                });
                                $('#saveField').spselectSet(currentRow.saveField || fieldData[0].value);
                            });
                        }
            ...
            if (!gridSetting.fieldMap[gridSetting.dbId + gridSetting.tableName]) {
                sp.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: gridSetting.dbId, tableName: gridSetting.tableName }, function (data) {
