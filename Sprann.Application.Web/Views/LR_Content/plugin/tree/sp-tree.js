(function ($, sp) {
    ...
    $.sptree = {
        ...
        render: function ($self) {
            var dfop = $self[0]._sptree.dfop;
            // 渲染成树
            var $treeRoot = $('<ul class="sp-tree-root" ></ul>');
            var _len = dfop.data.length;
            for (var i = 0; i < _len; i++) {
                var $node = $.sptree.renderNode(dfop.data[i], 0, i, dfop);
                $treeRoot.append($node);
            }
            $self.append($treeRoot);
            $self.spscroll();

            if (dfop.initialCallback && $.isFunction(dfop.initialCallback))  //渲染完成回调响应: 2018-12-28, Ridge W.
                dfop.initialCallback(dfop.data);
        ...
        renderNode: function (node, deep, path, dfop) {
            ...
            var $node = $('<li class="sp-tree-node"></li>');
            var $nodeDiv = $('<div id="' + dfop.id + '_' + nid + '" tpath="' + path + '" title="' + title + '"  dataId="' + dfop.id + '"  class="sp-tree-node-el" ></div>');
            if (node.hasChildren) {
                var c = (node.isexpand || dfop.isAllExpand) ? 'sp-tree-node-expanded' : 'sp-tree-node-collapsed';
                $nodeDiv.addClass(c);
            }
            else {
                $nodeDiv.addClass('sp-tree-node-leaf');
            }
            // span indent
            var $span = $('<span class="sp-tree-node-indent"></span>');
            if (deep == 1) {
                $span.append('<img class="sp-tree-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            }
            else if (deep > 1) {
                $span.append('<img class="sp-tree-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            ...
            $nodeDiv.append($span);
            // img
            var $img = $('<img class="sp-tree-ec-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            $nodeDiv.append($img);
            // checkbox
            if (node.showcheck) {
                ...
                var $checkBox = $('<img  id="' + dfop.id + '_' + nid + '_cb" + class="sp-tree-node-cb" src="' + dfop.cbiconpath + dfop.icons[node.checkstate] + '" />');
            ...
            // a
            var ahtml = '<a class="sp-tree-node-anchor" href="javascript:void(0);">';
            ahtml += '<span data-value="' + node.id + '" class="sp-tree-node-text" >' + node.text + '</span>';
            ahtml += '</a>';
            $nodeDiv.append(ahtml);
            // 节点事件绑定
            $nodeDiv.on('click', $.sptree.nodeClick);

            if (!node.complete) {
                $nodeDiv.append('<div class="sp-tree-loading"><img class="sp-tree-ec-icon" src="' + dfop.cbiconpath + 'loading.gif"/></div>');
            }

            $node.append($nodeDiv);
            if (node.hasChildren) {
                var $treeChildren = $('<ul  class="sp-tree-node-ct" >');
                if (!node.isexpand && !dfop.isAllExpand) {
                ...
                if (node.ChildNodes) {
                    ...
                    for (var k = 0; k < l; k++) {
                        ...
                        var $childNode = $.sptree.renderNode(node.ChildNodes[k], deep + 1, path + "." + k, dfop);
        ...
        renderNodeAsync: function ($this, node, dfop) {
            ...
            if (node.ChildNodes) {
                ...
                for (var k = 0; k < l; k++) {
                    node.ChildNodes[k].parent = node;
                    var $childNode = $.sptree.renderNode(node.ChildNodes[k], node._deep + 1, node._path + "." + k, dfop);
                    $treeChildren.append($childNode);
                }
        ...
        renderToo: function ($self) {
            var dfop = $self[0]._sptree.dfop;
            // 渲染成树
            var $treeRoot = $self.find('.sp-tree-root');
            $treeRoot.html('');
            var _len = dfop.data.length;
            for (var i = 0; i < _len; i++) {
                var $node = $.sptree.renderNode(dfop.data[i], 0, i, dfop);
        ...
        nodeClick: function (e) {
            ...
            var dfop = $parent[0]._sptree.dfop;
            if (et.tagName == 'IMG') {
                var $et = $(et);
                var $ul = $this.next('.sp-tree-node-ct');
                if ($et.hasClass("sp-tree-ec-icon")) {
                    if ($this.hasClass('sp-tree-node-expanded')) {
                        $ul.slideUp(200, function () {
                            $this.removeClass('sp-tree-node-expanded');
                            $this.addClass('sp-tree-node-collapsed');
                        });
                    }
                    else if ($this.hasClass('sp-tree-node-collapsed')) {
                        // 展开
                        var path = $this.attr('tpath');
                        var node = $.sptree.getItem(path, dfop);
                        if (!node.complete) {
                            if (!node._loading) {
                                node._loading = true;// 表示正在加载数据
                                $this.find('.sp-tree-loading').show();
                                sp.httpAsync('GET', dfop.url, { parentId: node.id }, function (data) {
                                    if (!!data) {
                                        node.ChildNodes = data;
                                        $ul = $.sptree.renderNodeAsync($this, node, dfop);
                                        $ul.slideDown(200, function () {
                                            $this.removeClass('sp-tree-node-collapsed');
                                            $this.addClass('sp-tree-node-expanded');
                                        });
                                        node.complete = true;
                                        $this.find('.sp-tree-loading').hide();
                                    }
                                    node._loading = false;
                                });
                            }
                ...
                else if ($et.hasClass("sp-tree-node-cb")) {
                    var path = $this.attr('tpath');
                    var node = $.sptree.getItem(path, dfop);
                    ...
                    $.sptree.checkChild($.sptree.check, node, node.checkstate, dfop);
                    $.sptree.checkParent($.sptree.check, node, node.checkstate, dfop);
                    if (!!dfop.nodeCheck) {
                ...
                $('#' + dfop.id).find('.sp-tree-selected').removeClass('sp-tree-selected');
                $this.addClass('sp-tree-selected');
                if (!!dfop.nodeClick) {
            ...
            if (fn(node, state, 1, dfop) != false) {
                if (node.ChildNodes != null && node.ChildNodes.length > 0) {
                    ...
                    for (var i = 0, len = cs.length; i < len; i++) {
                        $.sptree.checkChild(fn, cs[i], state, dfop);
                    }
                }
            }
        },
        ...
        search: function (keyword, data) {
            ...
            $.each(data, function (i, row) {
                ...
                if (!sp.validator.isNotNull(keyword).code || row.text.indexOf(keyword) != -1) {
                    
                    flag = true;
                }
                if (row.hasChildren) {
                    if ($.sptree.search(keyword, row.ChildNodes)) {
                        flag = true;
                    }
                }
                if (flag) {
        ...
        addNode: function ($self, node, Id, index) {// 下一版本完善
            ...
            if (!!Id)// 在最顶层
            {
                ...
                var $node = $.sptree.renderNode(node, 0, index, dfop);
                if ($self.find('.sp-tree-root>li').length == 0) {
                    $self.find('.sp-tree-root>li').append($node);
                }
                else {
                    $self.find('.sp-tree-root>li').eq(index).before($node);
                }
                ...
                var $node = $.sptree.renderNode(node, 0, path + '.' + index, dfop);
    ...
    $.fn.sptree = function (settings) {
        var dfop = {
            ...
            data: [],
            isAllExpand: false,
            cbiconpath: top.$.rootUrl + '/Content/images/sptree/',
            // 点击事件（节点信息）,节点$对象
            nodeClick: false,
            // 选中事件（节点信息）,节点$对象
            nodeCheck: false,
            initialCallback:false //初始化完成回调  2018-12-28，Ridge
        };
        $.extend(dfop, settings);
        var $self = $(this);
        dfop.id = $self.attr("id");
        if (dfop.id == null || dfop.id == "") {
            dfop.id = "sptree" + new Date().getTime();
            $self.attr("id", dfop.id);
        }
        $self.html('');
        $self.addClass("sp-tree");
        $self[0]._sptree = { dfop: dfop };
        $self[0]._sptree.dfop.backupData = dfop.data;
        if (dfop.url) {
            sp.httpAsync(dfop.method, dfop.url, dfop.param, function (data) {
                $self[0]._sptree.dfop.data = data || [];
                $self[0]._sptree.dfop.backupData = $self[0]._sptree.dfop.data;
                $.sptree.render($self);
            });
        }
        else {
    ...
    $.fn.sptreeSet = function (name,op) {
        ...
        switch (name) {
            case 'allNoCheck':
                $self.find('.sp-tree-node-cb').attr('src', dfop.cbiconpath + 'checkbox_0.png');
                setNoCheck(dfop.data);
                break;
            case 'allCheck':
                $self.find('.sp-tree-node-cb[src$="checkbox_0.png"]').trigger('click');
                break;
            case 'setCheck':
                var list = op;
                $.each(list, function (id, item) {
                    ...
                    if ($div.next().length == 0) {
                        $div.find('.sp-tree-node-cb').trigger('click');
                    }
                });
                break;
            case 'setValue':
            ...
            case 'getCheckNodeIdsByPath':
                ...
                break;
            case 'search':
                $.sptree.search(op.keyword, dfop.data);
                if (sp.validator.isNotNull(op.keyword).code) {
                    dfop._isSearch = true;
                }
                else if (dfop._isSearch) {
                    dfop._isSearch = false;
                }
                $.sptree.renderToo($self);
                break;
            case 'refresh':
                $.extend(dfop, op || {});
                if (!!dfop.url) {
