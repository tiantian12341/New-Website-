(function ($, sp) {
    ...
    $.spworkflow = {
        render: function ($self) {
            ...
            $self.addClass('sp-workflow');
            // 添加工具栏
            if (!dfop.isPreview) {
                var $tool = $('<div class="sp-workflow-tool" ></div>');
                $tool[0].dfop = dfop;
                $tool.append('<a  type="cursor" class="sp-workflow-btndown" id="' + dfop.id + '_btn_cursor" title="' + dfop.nodeRemarks.cursor + '" ><b class="ico_cursor" /></a>');
                $tool.append('<a  type="direct" class="sp-workflow-btn" id="' + dfop.id + '_btn_direct" title="' + dfop.nodeRemarks.direct + '" ><b class="ico_direct"/></a>');
                var toolbtnlen = dfop.toolBtns.length;
                if (toolbtnlen > 0) {
                    $tool.append('<span></span>');
                    for (var i = 0; i < toolbtnlen; ++i) {
                        var btn = dfop.toolBtns[i];
                        $tool.append('<a  type="' + btn + '" id="' + dfop.id + '_btn_' + btn + '" class="sp-workflow-btn" title="' + dfop.nodeRemarks[btn] + '" ><b class="ico_' + btn + '"/></a>');//加入自定义按钮
                    }
                }
                ...
                $tool.on("click", function (e) {
                    ...
                    $.spworkflow.switchToolBtn(dfop, type);
                    return false;
                });
                $self.append($tool);
            }
            else {
                $self.addClass('sp-workflow-preview');
            }


            // 工作流画板(工作区域)
            $self.append('<div class="sp-workflow-work"></div>');
            var $workArea = $("<div class='sp-workflow-workinner' style='width:5000px;height:5000px'></div>")
                .attr({ "unselectable": "on", "onselectstart": 'return false', "onselect": 'document.selection.empty()' });

            $self.children(".sp-workflow-work").spscroll();
            $self.children(".sp-workflow-work").find('.sp-scroll-box').append($workArea);
            $self.children(".sp-workflow-work").find('.sp-scroll-box').css({ 'width': 5000, 'height': 5000 });

            $workArea[0].dfop = dfop;
            $.spworkflow.initDraw($workArea, dfop);
            $workArea.on("click", $.spworkflow.clickWorkArea);

            $.spworkflow.initNodeEvent($workArea);
            $.spworkflow.initLineEvent($workArea);
            

            //对结点进行移动或者RESIZE时用来显示的遮罩层
            var $ghost = $("<div class='sp-workflow-rsghost'></div>").attr({ "unselectable": "on", "onselectstart": 'return false', "onselect": 'document.selection.empty()' });
            $self.append($ghost);
           
            var $lineMove = $('<div class="sp-workflow-linemover" style="display:none" ></div>');//操作折线时的移动框
            ...
            $lineMove.on("mousedown", { $workArea: $workArea }, function (e) {
                ...
                var ev = $.spworkflow.mousePosition(e), t = $.spworkflow.getElCoordinate($workArea[0]);
                var X, Y;
                X = ev.x - t.left;
                Y = ev.y - t.top;
                var p = lm.position();
                ...
                document.onmousemove = function (e) {
                    if (!e) e = window.event;
                    var ev = $.spworkflow.mousePosition(e);
                    var ps = lm.position();
                    X = ev.x - t.left;
                    Y = ev.y - t.top;
                    if (lm.data("type") == "sp") {
                ...
                document.onmouseup = function (e) {
                    ...
                    if (isMove) {
                        var p = lm.position();
                        if (lm.data("type") == "sp")
                            $.spworkflow.setLineM(lineId, p.left + 3);
                        else if (lm.data("type") == "tb")
                            $.spworkflow.setLineM(lineId, p.top + 3);
                    }
                    lm.css({ "background-color": "transparent" });
                    if (dfop.focusId == lm.data("tid")) {
                        $.spworkflow.focusItem(lm.data("tid"));
                    }
            ...
            // 设置线条工具条选定线时显示的操作框
            var $lineOper = $("<div class='sp-workflow-lineoper' style='display:none'><b class='sp'></b><b class='tb'></b><b class='sl'></b><b class='x'></b></div>");
            ...
            $lineOper.on("click", function (e) {
                ...
                if (type == 'x') {
                    $.spworkflow.delLine(id);
                    this.style.display = "none";
                }
                else {
                    $.spworkflow.setLineType(id, type);
                }
            });
        },
        switchToolBtn: function (dfop, type) {
            ...
            $oldBtn.removeClass('sp-workflow-btndown');
            $oldBtn.addClass('sp-workflow-btn');

            $newBtn.removeClass('sp-workflow-btn');
            $newBtn.addClass('sp-workflow-btndown');
        ...
        initDraw: function ($workArea, dfop) {
            ...
            defs.appendChild($.spworkflow.getSvgMarker("arrow1", "gray"));
            defs.appendChild($.spworkflow.getSvgMarker("arrow2", "#ff3300"));
            defs.appendChild($.spworkflow.getSvgMarker("arrow3", "#225ee1"));
        ...
        clickWorkArea: function (e) {
            ...
            if (!dfop.isPreview) {
                ...
                if (type == "cursor") {
                    ...
                    if (n == "svg" || (n == "DIV" && t.prop("class").indexOf("sp-workflow-workinner") > -1)) {
                        $.spworkflow.blurItem(dfop);
                    }
                    return;
                }
                else if (type == "direct") {
                ...
                var X, Y;
                var ev = $.spworkflow.mousePosition(e), t = $.spworkflow.getElCoordinate(this);
                ...
                if (type == 'endround') {
                    name = "结束";
                    if (dfop.hasEndround) {
                        sp.alert.error('只能有一个结束节点');
                        return false;
                    }
                }
                $.spworkflow.addNode($this, dfop, { id: sp.newGuid(), name: name, left: X, top: Y, type: type });
            }
        },
        // 取消所有结点/连线被选定的状态
        blurItem: function (dfop) {
            if (dfop.focusId != "") {
                ...
                if ($item.prop("tagName") == "DIV") {
                    $item.removeClass("sp-workflow-nodefocus");
                    $item.find('.sp-workflow-nodeclose').hide();
                    $item.removeClass("sp-workflow-nodemark");
                }
                else {
                    var lineData = $.spworkflow.getLine(dfop, dfop.focusId);
                    ...
                    var $lineOper = $('.sp-workflow-lineoper');
                    var $lineMove = $('.sp-workflow-linemover');
        ...
        focusItem: function (id) {
            ...
            if ($item.prop("tagName") == "DIV") {
                ...
                $item.addClass("sp-workflow-nodefocus");
                $item.find('.sp-workflow-nodeclose').show();
            }
            else {//如果是连接线
                ...
                var lineData = $.spworkflow.getLine(dfop, id);
            ...
            dfop.focusId = id;
            $.spworkflow.switchToolBtn(dfop, "cursor");
        },
        ...
        addNode: function ($workArea, dfop, node, isold) {
            ...
            if (mark == "conditionnode")
            {
                node.width = 160;
                node.height = 90;
                $node = $('<div class="sp-workflow-node item-conditionnode" id="' + node.id + '" ><div class="sp-workflow-nodeico"></div><b class="ico_' + node.type + 'div"></b><div class="sp-workflow-nodetext">' + node.name + '</div><div class="sp-workflow-nodeassemble" ></div></div>');

            }
            else if (mark != "startround" && mark != "endround") {
                $node = $('<div class="sp-workflow-node" id="' + node.id + '" ><div class="sp-workflow-nodeico"><b class="ico_' + node.type + '"></b></div><div class="sp-workflow-nodetext">' + node.name + '</div><div class="sp-workflow-nodeassemble" ></div></div>');
            }
            else {
                node.width = 52;
                node.height = 52;
                ...
                $node = $('<div class="sp-workflow-node item-' + mark + '" id="' + node.id + '" ><div class="sp-workflow-nodeico"></div><div class="sp-workflow-nodetext">' + node.name + '</div><div class="sp-workflow-nodeassemble" ></div></div>');
            }
            $node.find('.sp-workflow-nodeassemble').append('<div class="sp-workflow-nodeclose"></div>');
            $node.find('.sp-workflow-nodeassemble').append('<div class="sp-workflow-nodespot left"><div class="sp-workflow-nodespotc"></div></div>');
            $node.find('.sp-workflow-nodeassemble').append('<div class="sp-workflow-nodespot top"><div class="sp-workflow-nodespotc"></div></div>');
            $node.find('.sp-workflow-nodeassemble').append('<div class="sp-workflow-nodespot right"><div class="sp-workflow-nodespotc"></div></div>');
            $node.find('.sp-workflow-nodeassemble').append('<div class="sp-workflow-nodespot bottom"><div class="sp-workflow-nodespotc"></div></div>');
            ...
            if (node.state != undefined && (node.type == 'startround' || node.type == 'auditornode' || node.type == 'stepnode' || node.type == 'confluencenode')) {
                $node.css({ 'padding-left': '0','color':'#fff' }).find('.sp-workflow-nodeico').remove();
        ...
        updateNodeName: function ($workArea, nodeId) {
            ...
            var nodeData = $node[0].wfdata;
            $node.find('.sp-workflow-nodetext').html(nodeData.name);
        },
        initNodeEvent: function ($workArea) {
            var dfop = $workArea[0].dfop;

            //节点双击事件
            $workArea.delegate(".sp-workflow-node", "dblclick", { $workArea: $workArea }, function (e) {
                var $workArea = e.data.$workArea;
                var dfop = $workArea[0].dfop;
                var $node = $(this);
                var nodeData = $node[0].wfdata;
            ...
            if (!dfop.isPreview) {
                //绑定点击事件
                $workArea.delegate(".sp-workflow-node", "click", function (e) {
                    $.spworkflow.focusItem(this.id);
                    return false;
                });
                //绑定右击事件
                $workArea.delegate(".sp-workflow-node", "contextmenu", function (e) {
                    //$.spworkflow.focusItem(this.id);
                    //return false;
                });
                //绑定用鼠标移动事件(节点拖动)
                $workArea.delegate(".sp-workflow-nodeico", "mousedown", { $workArea: $workArea }, function (e) {
                    var $node = $(this).parents(".sp-workflow-node");
                    var dfop = $node[0].dfop;
                    var nodeData = $node[0].wfdata;
                    ...
                    var id = $node.attr("id");
                    var $workArea = e.data.$workArea;
                    $.spworkflow.focusItem(id);
                    var ev = $.spworkflow.mousePosition(e), t = $.spworkflow.getElCoordinate($workArea[0]);
                    var $ghost = $('#' + dfop.id).find('.sp-workflow-rsghost');
                    ...
                    $ghost.find('.sp-workflow-nodeclose').remove();
                    var X, Y;
                    ...
                    document.onmouseup = function (e) {
                        if (isMove) $.spworkflow.moveNode(id, X - t.left, Y + 47 - t.top);
                        $ghost.empty().hide();
                        document.onmousemove = null;
                        document.onmouseup = null;
                    }
                ...
                //绑定鼠标覆盖/移出事件
                $workArea.delegate(".sp-workflow-node", "mouseenter", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).addClass("sp-workflow-nodemark");
                });
                $workArea.delegate(".sp-workflow-node", "mouseleave", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).removeClass("sp-workflow-nodemark");
                });

                $workArea.delegate(".sp-workflow-nodespot", "mouseenter", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).addClass("sp-workflow-nodespotmark");
                });
                $workArea.delegate(".sp-workflow-nodespot", "mouseleave", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).removeClass("sp-workflow-nodespotmark");
                });

                //绑定连线时确定初始点
                $workArea.delegate(".sp-workflow-nodespot", "mousedown", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    var $this = $(this);
                    var $node = $this.parents('.sp-workflow-node');
                    ...
                    var line = $.spworkflow.drawLine('1',"sp_workflow_tmp_line", [X, Y], [X, Y], true, true);
                ...
                $workArea.mousemove(function (e) {
                    ...
                    X = ev.x - t.left;
                    Y = ev.y - t.top;
                    var line = document.getElementById("sp_workflow_tmp_line");
                ...
                $workArea.mouseup(function (e) {
                    ...
                    $("#sp_workflow_tmp_line").remove();
                });
                //绑定连线时确定结束点
                $workArea.delegate(".sp-workflow-nodespot", "mouseup", { $workArea: $workArea }, function (e) {
                    var $workArea = e.data.$workArea;
                    var dfop = $workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    var $this = $(this);
                    ...
                    if (lineStart) $.spworkflow.addLine(dfop, { id: sp.newGuid(), from: lineStart.id, to: nodeData.id, sp: lineStart.position, ep: position, name: "" });
                });
                //绑定结点的删除功能
                $workArea.delegate(".sp-workflow-nodeclose", "click", function () {
                    var $node = $(this).parents('.sp-workflow-node');
                    var nodeData = $node[0].wfdata;
                    var dfop = $node[0].dfop;
                    $.spworkflow.delNode(dfop, nodeData);
                    return false;
                });
            }
        },
        initLineEvent: function ($workArea) {
            ...
            if (!dfop.isPreview) {
                ...
                $workArea.delegate('g', "dblclick", { $workArea: $workArea }, function (e) {
                    ...
                    var lineData = $.spworkflow.getLine(dfop, this.id);
        ...
        addLine: function (dfop, line) {
            ...
            // 获取开始和结束节点的坐标
            var sxy = $.spworkflow.getLineSpotXY(line.from, dfop, line.sp);
            var exy = $.spworkflow.getLineSpotXY(line.to, dfop, line.ep);
            line.name = line.name || '';
            line.wftype = line.wftype || '1';
            dfop.line.push(line);

            if (line.type && line.type != "sl") {
                var res = $.spworkflow.calcPolyPoints(sxy, exy, line.type, line.M);
                $line = $.spworkflow.drawPoly(line.wftype, line.id, res.start, res.m1, res.m2, res.end, line.mark);
            }
            else {
                line.type = "sl";//默认为直线
                $line = $.spworkflow.drawLine(line.wftype, line.id, sxy, exy, line.mark);
            }
            var $draw = $('#' + dfop.id).find('svg');
        ...
        resetLines: function (nodeId, dfop) {
            ...
            for (var i = 0, l = dfop.line.length; i < l; i++) {
                ...
                if (line.from == nodeId || line.to == nodeId) {
                    ...
                    sxy = $.spworkflow.getLineSpotXY(line.from, dfop, line.sp);
                    exy = $.spworkflow.getLineSpotXY(line.to, dfop, line.ep);
                    ...
                    var lineData = $.spworkflow.getLine(dfop, lineId);
                    $($line).find('text').html(lineData.name);
                }
            }
        },
        //重新设置连线的样式 newType= "sl":直线, "sp":中段可左右移动型折线, "tb":中段可上下移动型折线
        setLineType: function (id, newType) {
            ...
            var sxy = $.spworkflow.getLineSpotXY(from, dfop, lineData.sp);
            var exy = $.spworkflow.getLineSpotXY(to, dfop, lineData.ep);

            var res;
            // 如果是变成折线
            if (newType != "sl") {
                var res = $.spworkflow.calcPolyPoints(sxy, exy, lineData.type, lineData.M);
                $.spworkflow.setLineM(id, $.spworkflow.getMValue(sxy, exy, newType), true);
            }
                // 如果是变回直线
            else {
                delete lineData.M;
                ...
                $line.remove();
                $line = $.spworkflow.drawLine(lineData.wftype, lineData.id, sxy, exy, lineData.mark);
                var $draw = $('#' + dfop.id).find('svg');
                $($line)[0].dfop = dfop;
                $draw.append($line);
                var lineData = $.spworkflow.getLine(dfop, id);
        ...
        setLineM: function (id, M, noStack) {
            ...
            var sxy = $.spworkflow.getLineSpotXY(from, dfop, lineData.sp);
            var exy = $.spworkflow.getLineSpotXY(to, dfop, lineData.ep);
            var ps = $.spworkflow.calcPolyPoints(sxy, exy, lineData.type, lineData.M);


            $('#' + id).remove();
            console.log(lineData);
            var $line = $.spworkflow.drawPoly(lineData.wftype, id, ps.start, ps.m1, ps.m2, ps.end, lineData.marked || dfop.focusId == id);
            var $draw = $('#' + dfop.id).find('svg');
    ...
    $.fn.spworkflow = function (op) {
        ...
        $self[0].dfop = dfop;
        $.spworkflow.render($self);
    };

    $.fn.spworkflowGet = function () {
        var $self = $(this);
        var $workArea = $self.find(".sp-workflow-workinner");
        var dfop = $workArea[0].dfop;
    ...
    $.fn.spworkflowSet = function (name, op) {
        var $self = $(this);
        var $workArea = $self.find(".sp-workflow-workinner");
        switch (name) {
            case 'updateNodeName':
                $.spworkflow.updateNodeName($workArea, op.nodeId);
                break;
            case 'updateLineName':
                $.spworkflow.updateLineName($workArea, op.lineId);
                break;
            case 'set':
                var dfop = $workArea[0].dfop;
                for (var i = 0, l = op.data.nodes.length; i < l; i++) {
                    var node = op.data.nodes[i];
                    $.spworkflow.addNode($workArea, dfop, node, true);
                }
                for (var i = 0, l = op.data.lines.length; i < l; i++) {
                    var line = op.data.lines[i];
                    $.spworkflow.addLine(dfop, line);
                }
                break;
        }
    }
