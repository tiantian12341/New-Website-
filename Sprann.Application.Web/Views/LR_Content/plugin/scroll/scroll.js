(function ($, sp, window) {
    ...
    var methods = {
        init: function ($this, callback) {
            ...
            $this.addClass('sp-scroll-wrap');
            // 加载内容
            var $content = $this.children();

            var $scroll = $('<div class="sp-scroll-box" id="' + id + '_box" ></div>');
            $this.append($scroll);
            $scroll.append($content);

            // 加载y滚动条
            var $vertical = $('<div class="sp-scroll-vertical"   ><div class="sp-scroll-vertical-block" id="' + id + '_vertical"></div></div>');
            $this.append($vertical);

            // 加载x滚动条
            var $horizontal = $('<div class="sp-scroll-horizontal" ><div class="sp-scroll-horizontal-block" id="' + id + '_horizontal"></div></div>');
        ...
        bindEvent: function ($this, $scroll, $vertical, $horizontal) { // 绑定监听事件
            ...
            // 监听鼠标移动
            $vertical.find('.sp-scroll-vertical-block').on('mousedown', function (e) {
                $move.show();
                var $this = $(this).parent().parent();
                var op = $this[0].op;
                op.isYMousedown = true;
                op.yMousedown = e.pageY;
                $this.addClass('sp-scroll-active');
                $this = null;
            });
            $horizontal.find('.sp-scroll-horizontal-block').on('mousedown', function (e) {
                $move.show();
                var $this = $(this).parent().parent();
                var op = $this[0].op;
            ...
            top.$(document).on('mousemove', { $obj: $this }, function (e) {
                ...
                $move.hide();
                e.data.$obj.removeClass('sp-scroll-active');
            });
        },
        ...
        updateY: function ($this) {
            ...
            if (op.sh > op.h) { // 出现纵向滚动条
                ...
                if ((op.sh - op.sy) < op.h) {
                    ...
                    //$scroll.css('transform', 'translateY(' + _sy + 'px)');
                }
                // 更新滚动条高度
                var scrollH = parseInt(op.h * op.h / op.sh);
                scrollH = (scrollH < 30 ? 30 : scrollH / 1.5);
                op.yh = scrollH;
        ...
        updateX: function ($this) {
            ...
            if (op.sw > op.w) {
                ...
                if ((op.sw - op.sx) < op.w) {
                    ...
                    //$scroll.css('transform', 'translateX(' + _sx + 'px)');
                }
                // 更新滚动条高度
                var scrollW = parseInt(op.w * op.w / op.sw);
                scrollW = (scrollW < 30 ? 30 : scrollW / 2);
                op.xw = scrollW;
        ...
        moveY: function ($this, isMousewheel) {
            ...
            if (isMousewheel) {
                ...
                $scroll.css({
                    'top': _sy + 'px'
                   // 'transform': 'translate(' + (-op['sx']) + 'px,' + _sy + 'px)',
                });
                $vertical.css({
                    'top': _y + 'px'
                    //'transform': 'translateY(' + _y + 'px)',
                ...
                $vertical.css({
                    'top': op.blockY + 'px'
                    //'transform': 'translateY(' + op.blockY + 'px)',
                });
            }
            op.callback && op.callback(op.sx, op.sy);
            $scroll = null;
        ...
        moveX: function ($this, isMousewheel) {
            ...
            if (isMousewheel) {
                ...
                $scroll.css({
                    'left': _sx + 'px'
                    //'transform': 'translate(' + _sx + 'px,' + (-op['sy']) + 'px)',
                });
                $horizontal.css({
                    'left': _x + 'px'
                    //'transform': 'translateX(' + _x + 'px)',
                ...
                $horizontal.css({
                    'left': op.blockX + 'px'
                    //'transform': 'translateX(' + op.blockX + 'px)',
                });
            }
            op.callback && op.callback(op.sx, op.sy);
            $scroll = null;
    ...
    $.fn.spscrollSet = function (name, data) {
        switch (name) {
            case 'moveRight':
                var $this = $(this);
                setTimeout(function () {
