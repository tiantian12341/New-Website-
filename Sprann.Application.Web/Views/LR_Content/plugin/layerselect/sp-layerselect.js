(function ($, sp) {
    ...
    $.splayerselect = {
        init: function ($self) {
            ...
            $self.addClass('sp-layerselect');
            $self.attr('type', 'splayerselect');
            var $input = $('<span>' + dfop.placeholder + '</span><i class="fa ' + dfop.icon + '"></i><div class="clear-btn" >清空</div>');
            $self.on('click', $.splayerselect.click);
            $self.html($input);
        },
        click: function (e) {
            var $self = $(this);
            ...
            if ($et.hasClass('clear-btn')) {
                ...
                top.splayerSelect = top.splayerSelect || {};
                top.splayerSelect[dfop.id] = dfop;
                ...
                sp.layerForm({
                    ...
                    callBack: function (id) {
                        return top[id].acceptClick($.splayerselect.callback);
                    }
                });
            }
        },
    ...
    $.fn.splayerselect = function (op) {
        ...
        $.splayerselect.init($self);
        return $self;
    };

    $.fn.splayerselectGet = function () {
        var $self = $(this);
        return $self[0].dfop._value;
    };
    $.fn.splayerselectSet = function (value) {
        var $self = $(this);
        var op = $self[0].dfop;
        if (value == '') {
            op._value = '';
