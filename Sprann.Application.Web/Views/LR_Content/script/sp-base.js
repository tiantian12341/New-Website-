top.sp = (function ($) {
    ...
    var sp = {
        ...
        loading: function (isShow, _text) {//加载动画显示与否
            var $loading = top.$("#sp_loading_bar");
            if (!!_text) {
                top.sp.language.get(_text, function (text) {
                    top.$("#sp_loading_bar_message").html(text);
                });
               
            } else {
                top.sp.language.get("正在拼了命为您加载…", function (text) {
                    top.$("#sp_loading_bar_message").html(text);
                });
            }
            if (isShow) {
        ...
        spLoading: function (isShow, title) {
            title = title || '加载中';
            let el = top.$("#sp-loading");
            el.find('#loading-title').text(title);
            if (isShow) {
                el.addClass('show');
            } else {
                el.removeClass('show');
        ...
        countFileSize: function (size) {
            if (size < 1024.00)
                return sp.toDecimal(size) + " 字节";
            else if (size >= 1024.00 && size < 1048576)
                return sp.toDecimal(size / 1024.00) + " KB";
            else if (size >= 1048576 && size < 1073741824)
                return sp.toDecimal(size / 1024.00 / 1024.00) + " MB";
            else if (size >= 1073741824)
                return sp.toDecimal(size / 1024.00 / 1024.00 / 1024.00) + " GB";
        },
        // 数组复制
        ...
        checkrow: function (id) {
            var isOK = true;
            if (id == undefined || id == "" || id == 'null' || id == 'undefined' || id == "0") {
                isOK = false;
                top.sp.language.get('您没有选中任何数据项,请选中后再操作！', function (text) {
                    sp.alert.warning(text);
                });
        ...
        alert: {
            success: function (msg) {
                top.sp.language.get(msg, function (text) {
                    toastr.success(text);
                });
