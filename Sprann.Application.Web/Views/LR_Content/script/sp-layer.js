(function ($, sp) {
    ...
    $.extend(sp, {
        // 询问框
        layerConfirm: function (_msg, callback, caption) {
            top.sp.language.get(_msg, function (msg) {
        ...
        layerForm: function (op) {
            ...
            var r = top.layer.open({
                ...
                btn: dfop.btn,
                content: op.url,
                skin: dfop.btn == null ? 'sp-layer-nobtn' : 'sp-layer',
                success: function (layero, index) {
                    top['layer_' + dfop.id] = sp.iframe($(layero).find('iframe').attr('id'), top.frames);
                    layero[0].sp_layerid = 'layer_' + dfop.id;
                    //如果底部有按钮添加-确认并关闭窗口勾选按钮
                    if (!!dfop.btn && layero.find('.sp-layer-btn-cb').length == 0) {
                        top.sp.language.get('确认关闭窗口', function (text) {
                            layero.find('.layui-layer-btn').append('<div class="checkbox sp-layer-btn-cb" myIframeId="layer_' + dfop.id + '" ><label><input checked="checked" type="checkbox" >' + text + '</label></div>');
                        });
                        layero.find('.layui-layer-btn a').each(function () {
                            var $this = $(this);
                            var _text = $this.text();
        ...
        layerClose: function (name, index) {
            ...
            var $IsClose = layero.find('.layui-layer-btn').find(".sp-layer-btn-cb input");
            ...
            if (IsClose) {
                ...
                top[layero[0].sp_layerid].location.reload();
            }
