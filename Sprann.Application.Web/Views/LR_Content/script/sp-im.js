(function ($, sp) {
            ...
            if (imChat) {
                ...
                imChat.client.revMsg = function (userId, msg, dateTime, isSystem) {
                    if (!loadingMsg2[userId]) {
                        ...
                        sp.im.revMsg && sp.im.revMsg(userId, msg, dateTime, isSystem || 0);
                    }
                }
            }
        }
    ...
    sp.im = {
        addContacts: function (userId) {// 添加联系人
            sp.httpAsync('Post', top.$.rootUrl + '/LR_IM/IMMsg/AddContact', { otherUserId: userId }, function (data) {});
        },
        getContacts: function (callback) {// 获取最近的联系人列表
            setTimeout(function () {
                sp.httpAsync('GET', top.$.rootUrl + '/LR_IM/IMMsg/GetContactsList', {}, function (data) {
                    data = data || [];
                    _im.init();
                    callback(data);
                });
        ...
        updateContacts: function (userId) {
            sp.httpAsync('Post', top.$.rootUrl + '/LR_IM/IMMsg/UpdateContactState', { otherUserId: userId}, function (data) {
            });
        },
        sendMsg: function (userId, msg) {// 发送消息
            var time = "";
            var loginInfo = sp.clientdata.get(['userinfo']);
            var point = { userId: loginInfo.userId, content: msg, time: sp.getDate('yyyy-MM-dd hh:mm:ss'), isSystem: 0 };
            addMsgList(userId, point);
            sp.httpAsync('Post', top.$.rootUrl + '/LR_IM/IMMsg/SendMsg', { userId: userId, content: msg }, function (data) {
                _im.sendMsg(userId, msg);// 发送给即时通讯服务
            });
            if (msgList[userId].length > 1) {
                if (sp.parseDate(point.time).DateDiff('s', msgList[userId][msgList[userId].length - 2].time) > 60) {
                    time = point.time;
                }
            }
            else {
        ...
        getMsgList: function (userId, callback,isGetMsgList) {
            ...
            if (msgList[userId].length == 0 && isGetMsgList) {// 如果没有信息，获取最近10条的聊天记录
                ...
                sp.httpAsync('GET', top.$.rootUrl + '/LR_IM/IMMsg/GetMsgList', { userId: userId }, function (data) {
                    msgList[userId] = msgList[userId] || [];
                    data = data || [];
                    var len = data;
                    if (len > 0) {
