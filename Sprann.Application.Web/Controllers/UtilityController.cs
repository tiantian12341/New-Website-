...
namespace Sprann.Application.Web.Controllers
{
    ...
    public class UtilityController : MvcControllerBase
    {
        ...
        public void ExportExcel(string fileName, string columnJson, string dataJson, string exportField)
        {
            ...
            //行数据
            DataTable rowData = JosnToDataTable(dataJson);
        ...
        public  DataTable JosnToDataTable(string json)
        {
        ...
        [AjaxOnly]
        [HandlerLogin(FilterMode.Enforce)]
        public ActionResult TextFileContent(string urlPath)
        {
            string localPath = Server.MapPath(urlPath);
            if (!System.IO.File.Exists(localPath))
            {
                return Fail("文件不存在！");
            }
            //return new FileContentResult(System.IO.File.ReadAllBytes(localPath), "text/Plain");
            return Success(System.IO.File.ReadAllText(localPath));
        }

        [HandlerLogin(FilterMode.Enforce)]
        public ActionResult FileEditor()
        {
            ViewBag.fileUrl = Request["fileUrl"];
            return View();
        }

        [AjaxOnly]
        [HandlerLogin(FilterMode.Enforce)]
        public ActionResult SetFileContent(string urlPath, string content)
        {
            string localPath = Server.MapPath(urlPath);
            if (!System.IO.File.Exists(localPath))
            {
            ...
            return hasError ? Fail(errMsg) : Success("ok");
        }

        [AjaxOnly]
        [HandlerLogin(FilterMode.Enforce)]
        public ActionResult SetViewFileWidth(string urlPath, string fieldName, string newWidth)
        {
            string localPath = Server.MapPath(urlPath);
            if (!System.IO.File.Exists(localPath))
                return Fail("文件不存在！");

            string oldFileContent = System.IO.File.ReadAllText(localPath);

            //{ label: '模具编号', name: 'moe_mjbh', width: 100, align: "left" },
            string linePattern = "name:(\\s)*'" + fieldName + "',(\\s)*width:(\\s)*(?<width>\\d+)";
            string newFileContent = Regex.Replace(oldFileContent, linePattern,
                m => m.Value.Replace(m.Groups["width"].Value, newWidth),
            ...
            try
            {
                System.IO.File.WriteAllText(localPath, newFileContent, System.Text.Encoding.UTF8);
            }
            catch (Exception e)
            {
                hasError = true;
