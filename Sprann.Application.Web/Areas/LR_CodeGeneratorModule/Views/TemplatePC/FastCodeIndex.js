...
var bootstrap = function ($, sp) {
    ...
    var page = {
            ...
            $('#wizard').wizard().on('change', function (e, data) {
                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        if (!$('#step-2').spValidform()) {
                            return false;
                        }
                        var formData = $('#step-2').spGetFormData();
                        ...
                        sp.httpAsyncPost(top.$.rootUrl + '/LR_CodeGeneratorModule/TemplatePC/LookFastCode', postData, function (res) {
                            if (res.code == 200)
                            {
        ...
        /*保存数据*/
        save: function () {
            var moduleData = $('#step-4').spGetFormData();
            moduleData.F_EnabledMark = 1;
            postData.moduleEntityJson = JSON.stringify(moduleData);
            $.spSaveForm(top.$.rootUrl + '/LR_CodeGeneratorModule/TemplatePC/CreateFastCode', postData, function (res) {
