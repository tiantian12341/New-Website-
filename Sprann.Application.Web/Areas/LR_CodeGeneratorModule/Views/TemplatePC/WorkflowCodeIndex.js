...
var bootstrap = function ($, sp) {
    ...
    var page = {
                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        if (mapField[databaseLinkId + mainTable.name]) {
                            ...
                            $('#workfield').spselectRefresh({ data: mapField[databaseLinkId + mainTable.name] });
                        }
                        else {
                            if (!mapField[databaseLinkId + mainTable.name]) {
                                sp.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: databaseLinkId, tableName: mainTable.name }, function (data) {
                                    ...
                                    $('#workfield').spselectRefresh({ data: data });
                                });
                            }
                        }
                        ...
                        var workfield = $('#workfield').spselectGet();
                        if (!workfield) {
                            sp.alert.error('请选择流程关联字段！');
                            return false;
                        }
                    }
                    else if (data.step == 5) {
                        ...
                        var _colData = {
                            ...
                            workField: $('#workfield').spselectGet()
                        };
                        ...
                        sp.httpAsyncPost(top.$.rootUrl + '/LR_CodeGeneratorModule/TemplatePC/LookWorkflowCode', postData, function (res) {
                            if (res.code == 200) {
        ...
        save: function () {
            ...
            $.spSaveForm(top.$.rootUrl + '/LR_CodeGeneratorModule/TemplatePC/CreateWorkflowCode', postData, function (res) { }, true);
        }
    };
