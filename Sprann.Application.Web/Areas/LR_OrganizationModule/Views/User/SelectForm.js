...
var bootstrap = function ($, sp) {
    ...
    function renderSelectedUserlist() {
        ...
        if (!!selectValue)
        {
            sp.httpAsync('GET', top.$.rootUrl + '/LR_OrganizationModule/User/GetEntityListByUserIds', { keyValue: selectValue }, function (data) {
                if (!!data) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#department_tree').sptree({
                nodeClick: function (item) {
                    ...
                    if (!!userlist[item.id]) {
                        ...
                        sp.httpAsync('GET', top.$.rootUrl + '/LR_OrganizationModule/User/GetList', { companyId: companyId, departmentId: departmentId }, function (data) {
            ...
            // 公司
            $('#company_select').spCompanySelect().bind('change', function () {
                companyId = $(this).spselectGet();
                $('#department_tree').sptreeSet('refresh', {
            ...
            $("#txt_keyword").keydown(function (event) {
                if (event.keyCode == 13) {
                    ...
                    if (keyword != "") {
                        sp.httpAsync('GET', top.$.rootUrl + '/LR_OrganizationModule/User/GetList', { companyId: companyId, keyword: keyword }, function (data) {
                            renderUserlist(data || []);
                        });
                    }
                    else {
            ...
            $('#selected_user_list_warp').spscroll();
        },
        initData: function () {
            var companyId = request('companyId');
            var departmentId = request('departmentId');
            if (!!companyId) {
                $('#company_select').spselectSet(companyId);
                $('#department_tree').sptreeSet('setValue', departmentId);
            }
            else {
                var loginInfo = sp.clientdata.get(['userinfo']);
                if (!!loginInfo && !!loginInfo.F_CompanyId && !!loginInfo.F_DepartmentId) {
                    $('#company_select').spselectSet(loginInfo.F_CompanyId);
                    $('#department_tree').sptreeSet('setValue', loginInfo.F_DepartmentId);
                }
            }
            // 初始化选中的人员
