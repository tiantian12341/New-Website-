...
var bootstrap = function ($, sp) {
    ...
    var page = {
        ...
        initData: function () {
            if (!!keyValue) {
                $.spSetForm(top.$.rootUrl + '/LR_CodeDemo/CustomerInfo/GetFormData?keyValue=' + keyValue, function (data) {
                    for (var id in data) {
                        if (!!data[id].length && data[id].length > 0) {
                            $('#LR_Demo_OrderList').jfGridSet('refreshdata', { rowdatas: data[id] });
                        }
                        else {
                            $('[data-table="' + id + '"]').spSetFormData(data[id]);
                        }
                    }
                });
            }
    ...
    acceptClick = function (callBack) {
        ...
        var postData = {};
        postData.strEntity = JSON.stringify($('[data-table="LR_Demo_User"]').spGetFormData());
        postData.strlR_Demo_OrderListList = JSON.stringify($('#LR_Demo_OrderList').jfGridGet('rowdatas'));
        $.spSaveForm(top.$.rootUrl + '/LR_CodeDemo/CustomerInfo/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }
