...
var bootstrap = function ($, sp) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            // 收款方式
            $('#paymentState').spDataItemSelect({ code: 'Client_PaymentMode' });
            ...
            $('#sp_add').on('click', function () {
                sp.frameTab.open({ F_ModuleId: 'order_add', F_Icon: 'fa fa-file-text-o', F_FullName: '新增订单', F_UrlAddress: '/LR_CRMModule/CrmOrder/Form' });
            });
            // 编辑
            $('#sp_edit').on('click', function () {
                var keyValue = $('#gridtable').jfGridValue('F_OrderId');
                if (sp.checkrow(keyValue)) {
                    sp.frameTab.open({ F_ModuleId: 'order_add', F_Icon: 'fa fa-file-text-o', F_FullName: '新增订单', F_UrlAddress: '/LR_CRMModule/CrmOrder/Form?keyValue=' + keyValue });
                }
            });
            // 删除
            $('#sp_delete').on('click', function () {
                ...
                if (sp.checkrow(keyValue)) {
                    sp.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            sp.deleteForm(top.$.rootUrl + '/LR_CRMModule/CrmOrder/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
