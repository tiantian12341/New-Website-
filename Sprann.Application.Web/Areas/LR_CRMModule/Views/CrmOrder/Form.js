...
var bootstrap = function ($, sp) {
    ...
    var acceptClick = function (type) {// 0保存并新增 1保存
        if (!$('.sp-layout-wrap').spValidform()) {
            return false;
        }
        var formData = $('.sp-layout-wrap').spGetFormData(keyValue);
        var productData = [];
        ...
        sp.layerConfirm('注：您确认要保存此操作吗？', function (res, index) {
            if (res) {
                $.spSaveForm(top.$.rootUrl + '/LR_CRMModule/CrmOrder/SaveForm?keyValue=' + keyValue, postData, function (res) {
                    if (res.code == 200) {
                        if (type == 0) {
                            window.location.href = top.$.rootUrl + '/LR_CRMModule/CrmOrder/Form';
                        }
                        else {
                            sp.frameTab.close('order_add');
                        }
                    }
                });
    ...
    var page = {
        ...
        bind: function () {
            ...
            // 收款方式
            $('#F_PaymentMode').spDataItemSelect({ code: 'Client_PaymentMode' });

            // 合同附件
            $('#F_ContractFile').spUploader();
        ...
        initData: function () {
            if (!!keyValue) {
                $.spSetForm(top.$.rootUrl + '/LR_CRMModule/CrmOrder/GetFormData?keyValue=' + keyValue, function (data) {//
                    $('.sp-layout-wrap').spSetFormData(data.orderData);
