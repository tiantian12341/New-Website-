...
var bootstrap = function ($, sp) {
    ...
    save = function (processId, callBack, i) {
        ...
        $.spSaveForm(top.$.rootUrl + '/LR_FormModule/Custmerform/SaveInstanceForm?keyValue=' + keyValue + "&schemeInfoId=" + id + '&processIdName=' + processIdName, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                ...
                if (_ids.length == 1) {
                    if (field.isLook != 1) {// 如果没有查看权限就直接移除
                        ...
                        if (field.isEdit != 1) {
                            ...
                            if ($('#' + _ids[0]).hasClass('spUploader-wrap')) {
    ...
    setFormData = function (processId) {
        if (!!processId) {
            $.spSetForm(top.$.rootUrl + '/LR_FormModule/Custmerform/GetInstanceForm?schemeInfoId=' + id + '&keyValue=' + processId + '&processIdName=' + processIdName, function (data) {//
                page.setFormData(data);
            });
        }
    };
    ...
    getFormData = function () {
        return $('body').spGetCustmerformData(keyValue);
    }
}