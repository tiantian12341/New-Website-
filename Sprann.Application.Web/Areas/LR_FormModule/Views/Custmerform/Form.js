...
var bootstrap = function ($, sp) {
    ...
    var page = {
        ...
        bind: function () {
                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        if (dbId == '' || dbTable.length == 0) {
                            sp.alert.error('至少选择一张数据表');
                            return false;
                        }
                        $('#step-3').spCustmerFormDesigner('updatedb', { dbId: dbId, dbTable: dbTable });
            ...
            $('#F_Category').spDataItemSelect({ code: 'FormSort' });
            $('#F_Category').spselectSet(categoryId);
            // 新增
            $('#sp_db_add').on('click', function () {
                dbTable = $('#gridtable').jfGridGet('rowdatas');
                selectedRow = null;
                sp.layerForm({
                    id: 'DataTableForm',
            ...
            // 删除
            $('#sp_db_delete').on('click', function () {
                var keyValue = $('#gridtable').jfGridValue('name');
                if (sp.checkrow(keyValue)) {
            ...
            // 设计页面初始化
            $('#step-3').spCustmerFormDesigner('init');
            // 保存数据按钮
        ...
        initData: function () {
            if (!!keyValue) {
                $.spSetForm(top.$.rootUrl + '/LR_FormModule/Custmerform/GetFormData?keyValue=' + keyValue, function (data) {//
                    $('#step-1').spSetFormData(data.schemeInfoEntity);
                    var scheme = JSON.parse(data.schemeEntity.F_Scheme);
                    $('#F_DbId').spselectSet(scheme.dbId);
                    $('#gridtable').jfGridSet('refreshdata', scheme.dbTable);
                    $('#step-3').spCustmerFormDesigner('set', scheme);
                });
            }
        },
        /*保存数据*/
        save: function () {
            var schemeInfo = $('#step-1').spGetFormData(keyValue);
            if (!$('#step-3').spCustmerFormDesigner('valid')) {
                return false;
            }
            var scheme = $('#step-3').spCustmerFormDesigner('get');
            schemeInfo.F_Type = 0;
            ...
            $.spSaveForm(top.$.rootUrl + '/LR_FormModule/Custmerform/SaveForm', postData, function (res) {
                // 保存成功后才回调
