...
var bootstrap = function ($, sp) {
    ...
    var nowschemeId = sp.frameTab.currentIframe().nowschemeId;
    ...
    var page = {
        ...
        bind: function () {
            ...
            // 更新到此版本
            $('#sp_update').on('click', function () {
                var schemeId = $('#gridtable').jfGridValue('F_Id');
                if (sp.checkrow(schemeId)) {
                    if (schemeId != nowschemeId) {
                        sp.layerConfirm('是否要更新到该版本！', function (res) {
                            if (res) {
                                sp.postForm(top.$.rootUrl + '/LR_FormModule/Custmerform/UpdateScheme', { schemeInfoId: keyValue, schemeId: schemeId }, function () {
                                    nowschemeId = schemeId;
                                    sp.frameTab.currentIframe().nowschemeId = schemeId;
