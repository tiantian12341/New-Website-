...
var bootstrap = function ($, sp) {
    ...
    var page = {
        init: function () {
            // 获取自定义表单设置内容
            sp.httpAsync('GET', top.$.rootUrl + '/LR_FormModule/FormRelation/GetCustmerFormData', { keyValue: id }, function (data) {
                ...
                if (settingJson.query.isDate == '1' && !!settingJson.query.DateField) {// 时间搜索框
                    $('#datesearch').spdate({
                        ...
                        selectfn: function (begin, end) {
                            queryJson.spbegin = begin;
                            queryJson.spend = end;
                            queryJson.spdateField = settingJson.query.DateField;
                ...
                if (!!settingJson.query.fields && settingJson.query.fields.length > 0) {
                    $('#multiple_condition_query_item').html('<div id="multiple_condition_query"><div class="sp-query-formcontent"></div></div>');
                    $('#multiple_condition_query').spMultipleQuery(function (_queryJson) {
                        queryJson = _queryJson;
                        page.search();
                    }, 220);
                    var $content = $('#multiple_condition_query .sp-query-content');
                    ...
                    $.each(queryFieldMap, function (id, item) {
                        if (!!item.compont) {
                            var $row = $('<div class="col-xs-' + (12 / parseInt(item.portion)) + ' sp-form-item" ></div>');
                            var $title = $(' <div class="sp-form-item-title">' + item.fieldName + '</div>');
                            $row.append($title);
                            $content.append($row);
                            $.spFormComponents[item.compont.type].renderQuery(item.compont, $row)[0].compont = item.compont;
                        }
                    });
                }
                // 列表设置
        ...
        bind: function () {
            ...
            $('#sp_add').on('click', function () {
                if (settingJson.layer.opentype == '1') {// 窗口弹层页
                    ...
                    sp.frameTab.open({ F_ModuleId: id, F_Icon: 'fa fa-pencil-square-o', F_FullName: '新增', F_UrlAddress: '/LR_FormModule/Custmerform/TabInstanceForm?id=' + relation.F_FormId });
                }
            });
            // 编辑
            $('#sp_edit').on('click', function () {
                ...
                if (sp.checkrow(keyValue)) {
                    if (settingJson.layer.opentype == '1') {// 窗口弹层页
                        ...
                        sp.frameTab.open({ F_ModuleId: id, F_Icon: 'fa fa-pencil-square-o', F_FullName: '编辑', F_UrlAddress: '/LR_FormModule/Custmerform/TabInstanceForm?id=' + relation.F_FormId + "&keyValue=" + keyValue });
                    }
                }
            });
            // 删除
            $('#sp_delete').on('click', function () {
                ...
                if (sp.checkrow(keyValue)) {
                    sp.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            sp.deleteForm(top.$.rootUrl + '/LR_FormModule/Custmerform/DeleteInstanceForm', { keyValue: keyValue, schemeInfoId: relation.F_FormId }, function () {
                                refreshGirdData();
                            });
                        }
                    });
