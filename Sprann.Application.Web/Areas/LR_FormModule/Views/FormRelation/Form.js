...
var bootstrap = function ($, sp) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#wizard').wizard().on('change', function (e, data) {
                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        // 加载表单数据
                        var formId = $('#F_FormId').spselectGet();
                        if (setFormId != formId) {
                            queryData = [];
                            colData = [];
                            setFormId = formId;
                            ...
                            $.spSetForm(top.$.rootUrl + '/LR_FormModule/Custmerform/GetFormData?keyValue=' + formId, function (data) {//
                                ...
                                for (var i = 0, l = formscheme.data.length; i < l; i++) {
                                    ...
                                    for (var j = 0, jl = componts.length; j < jl; j++) {
                                        ...
                                        if (item.type != "gridtable" && item.type != "guid") {
                                            var point = { 'F_ModuleFormId': sp.newGuid(), 'F_EnCode': item.id, 'F_FullName': item.title };
                                ...
                                if (!!tableFields[formscheme.dbId + mainTable]) {
                                    $('#queryDatetime').spselectRefresh({
                                        data: tableFields[formscheme.dbId + mainTable]
                                    });
                                }
                                else {
                                    sp.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: formscheme.dbId, tableName: mainTable }, function (data) {
            ...
            // 新增
            $('#sp_add_query').on('click', function () {
                sp.layerForm({
                    id: 'QueryFieldForm',
                    title: '添加条件字段',
            ...
            // 编辑
            $('#sp_edit_query').on('click', function () {
            ...
            // 删除
            $('#sp_delete_query').on('click', function () {
                var id = $('#query_girdtable').jfGridValue('id');
                if (sp.checkrow(id)) {
                    sp.layerConfirm('是否确认删除该字段', function (res, index) {
                        if (res) {
                            for (var i = 0, l = queryData.length; i < l; i++) {
            ...
            // 删除
            $('#sp_delete_col').on('click', function () {
                var id = $('#col_girdtable').jfGridValue('id');
                if (sp.checkrow(id)) {
                    sp.layerConfirm('是否确认删除该字段', function (res, index) {
                        if (res) {
        ...
        initData: function () {
            if (!!keyValue) {
                $.spSetForm(top.$.rootUrl + '/LR_FormModule/FormRelation/GetFormData?keyValue=' + keyValue, function (data) {//
                    setFormId = data.relation.F_FormId;

                    $.spSetForm(top.$.rootUrl + '/LR_FormModule/Custmerform/GetFormData?keyValue=' + setFormId, function (data) {//
                        formscheme = JSON.parse(data.schemeEntity.F_Scheme);
                        // 获取表单字段
                        for (var i = 0, l = formscheme.data.length; i < l; i++) {
                            var componts = formscheme.data[i].componts;
                    ...
                    $('#step-1').spSetFormData(data.module);
                    $('#F_FormId').spselectSet(data.relation.F_FormId);
                    ...
                    $('#queryDatetime').spselectSet(settingJson.query.DateField);
        ...
        save: function () {
            ...
            var formdata = $('#step-1').spGetFormData(keyValue);
            ...
            for (var i = 0, l = colData.length; i < l; i++) {
                ...
                var point = { F_ModuleColumnId: sp.newGuid(), F_FullName: colData[i].fieldName, F_EnCode: code.toLowerCase() };
            ...
            $.spSaveForm(top.$.rootUrl + '/LR_FormModule/FormRelation/SaveForm?keyValue=' + keyValue, postData, function (res) {
                // 保存成功后才回调
