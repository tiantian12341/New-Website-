    ...
    var page = {
        ...
        bind: function () {
            ...
            $(".crumb-path span").click(function () {
                ...
                if (value == "back") {
                    ...
                    $.spSetForm(top.$.rootUrl + _url + '?folderId=' + folderId, function (data) {//
                        $('#gridTable').jfGridSet('refreshdata', data);
                    });
                    $(".crumb-path span:last").remove();
                } else {
                    $.spSetForm(top.$.rootUrl + _url + '?folderId=0', function (data) {//
                        $('#gridTable').jfGridSet('refreshdata', data);
                    });
                    $(".crumb-path .back").hide();
            ...
            //上传文件
            $('#sp-uploadify').on('click', function () {
                sp.layerForm({
                    id: 'UploadifyForm',
                    title: '上传文件',
            ...
            //新建文件夹
            $('#sp-addfolder').on('click', function () {
                sp.layerForm({
                    id: 'FolderForm',
                    title: '添加文件夹',
            ...
            //文件下载
            $('#sp-download').on('click', function () {
                var keyValue = $("#gridTable").jfGridValue("F_FileId");
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    if (fileType != 'folder') {
                        sp.download({ url: top.$.rootUrl + '/LR_OAModule/ResourceFile/DownloadFile', param: { keyValue: keyValue }, method: 'POST' });
                    } else {
                        sp.alert.error('目前不支持文件夹下载');
                    }
                } else {
                    sp.alert.warning('请选择要下载的文件！');
                }
            });
            //文件预览
            $('#sp-preview').on('click', function () {
                ...
                if (keyValue) {
                    if (fileType != 'folder') {
                        ...
                        sp.alert.error('请选择文件');
                    }
                } else {
                    sp.alert.warning('请选择要预览的文件！');
                }
            });
            //文件（夹）删除
            $('#sp-delete').on('click', function () {
                ...
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    sp.layerConfirm("注：您确定要删除此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？可在回收站还原！", function (res) {
                        if (res) {
                            sp.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/RemoveForm', { keyValue: keyValue, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                } else {
                    sp.alert.warning('请选择要删除的文件夹或文件！');
                }
            });
            //文件共享
            $('#sp-share').on('click', function () {
                var keyValue = $("#gridTable").jfGridValue("F_FileId");
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    sp.layerConfirm("注：您确定要共享此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？", function (res) {
                        if (res) {
                            sp.warning(top.$.rootUrl + '/LR_OAModule/ResourceFile/ShareFile', { keyValue: keyValue, IsShare: 1, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                } else {
                    sp.alert.warning('请选择要共享的文件夹或文件！');
                }
            });
            //取消共享
            $('#sp-cancelshare').on('click', function () {
                var keyValue = $("#gridTable").jfGridValue("F_FileId");
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    sp.layerConfirm("注：您确定要取消共享此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？", function (res) {
                        if (res) {
                            sp.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/ShareFile', { keyValue: keyValue, IsShare: 0, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                } else {
                    sp.alert.warning('请选择要取消共享的文件夹或文件！');
                }
            });
            //文件（夹）重命名
            $('#sp-rename').on('click', function () {
                var keyValue = $("#gridTable").jfGridValue("F_FileId");
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    sp.layerForm({
                    ...
                    sp.alert.warning('请选择要重命名的文件夹或文件！');
                }
            });
            //文件（夹）移动
            $('#sp-move').on('click', function () {
                var keyValue = $("#gridTable").jfGridValue("F_FileId");
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    sp.layerForm({
                    ...
                    sp.alert.warning('请选择要移动的文件夹或文件！');
                }
            });
            //文件（夹）还原
            $('#sp-restoreFile').on('click', function () {
                var keyValue = $("#gridTable").jfGridValue("F_FileId");
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    sp.layerConfirm("注：您确定要还原此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？", function (res) {
                        if (res) {
                            sp.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/RestoreFile', { keyValue: keyValue, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                } else {
                    sp.alert.warning('请选择要还原的文件夹或文件！');
                }
            });
            //文件（夹）彻底删除
            $('#sp-thoroughDelete').on('click', function () {
                var keyValue = $("#gridTable").jfGridValue("F_FileId");
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    sp.layerConfirm("注：您确定要删除此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？该操作将无法恢复！", function (res) {
                        if (res) {
                            sp.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/ThoroughRemoveForm', { keyValue: keyValue, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            //清空回收站
            $('#sp-emptyRecycled').on('click', function () {
                sp.layerConfirm("注：您确定要清空回收站吗？该操作将无法恢复！", function (res) {
                    if (res) {
                        sp.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/EmptyRecycledForm', {}, function () {
                            refreshGirdData();
                        });
                    }
                });
        ...
        initleft: function () {
            $('#sp_left_list li').on('click', function () {
                ...
                $("#sp-more").hide();
                $("#sp-restoreFile").hide();
                $("#sp-thoroughDelete").hide();
                $("#sp-emptyRecycled").hide();
                $(".crumb-path .back").hide();
                $(".crumb-path .add").remove();
                ...
                switch (data_value) {
                    ...
                    case "recycledFile":
                        ...
                        $("#sp-restoreFile").show();
                        $("#sp-thoroughDelete").show();
                        $("#sp-emptyRecycled").show();
                        break;
                    case "myShare":
                ...
                $.spSetForm(top.$.rootUrl + _url, function (res) {//
                    if (res.length != 0) {
            ...
            $('#gridTable').jfGrid({
                ...
                dblclick: function (id) {
                    ...
                    if (rowData.F_FileType == "folder") {
                        ...
                        $.spSetForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/GetListJson?folderId=' + fileId, function (data) {//
                            $('#gridTable').jfGridSet('refreshdata', data);
                        });
                        ...
                        $(".crumb-path span.add").click(function () {
                            $(this).nextAll().remove();
                            $.spSetForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/GetListJson?folderId=' + $(this).attr('data-fileId'), function (data) {//
                                $('#gridTable').jfGridSet('refreshdata', data);
                            });
                        });
                    }
