...
namespace Sprann.Application.Web.Areas.LR_SystemModule.Controllers
{
    ...
    public class DatabaseTableController : MvcControllerBase
    {
        ...
        /// 执行存储过程
        /// </summary>
        /// <param name="dbLinkId">数据连接名称</param>
        /// <param name="spName">存储过程名称</param>
        /// <param name="spArgs">存储过程参数JSON序列化</param>
        /// <returns></returns>
        [HttpPost]
        [AjaxOnly]
        public ActionResult ExecuteSp(string dbLinkId, string spName, string spArgs)
        {
            List<SpParameterModel> sparamList = spArgs.ToObject<List<SpParameterModel>>();
            var data = databaseTableIBLL.ExecuteSpTableResult(dbLinkId, spName, sparamList);
            return Success(data);
        }
        #endregion
    }
