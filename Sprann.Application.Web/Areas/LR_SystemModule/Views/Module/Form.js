...
var bootstrap = function ($, sp) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            // 目标
            $('#F_Target').spselect().on('change', function () {
                // 目标改变
        ...
        initGrid: function () {
            $('#btns_girdtable').jfGrid({
                ...
                onAddRow: function (row, rows) {
                    row.F_ModuleButtonId = sp.newGuid();
                    row.F_ParentId = '';
                }
            });
            $('#view_girdtable').jfGrid({
                ...
                isEdit: true,
                onAddRow: function (row, rows) {
                    row.F_ModuleColumnId = sp.newGuid();
                    row.F_ParentId = '0';
                }
            });
        ...
        initData: function () {
            if (!!keyValue) {
                $.spSetForm(top.$.rootUrl + '/LR_SystemModule/Module/GetFormData?keyValue=' + keyValue, function (data) {//
                    $('#step-1').spSetFormData(data.moduleEntity);
                ...
                $('#F_ParentId').spselectSet(moduleId);
            }
        },
        /*保存数据*/
        save: function () {
            ...
            $.spSaveForm(top.$.rootUrl + '/LR_SystemModule/Module/SaveForm', postData, function (res) {
                // 保存成功后才回调
                sp.frameTab.currentIframe().refreshGirdData(formdata);
            });
        }
    };
