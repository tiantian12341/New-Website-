...
var bootstrap = function ($, sp) {
    ...
    var page = {
        ...
        bind: function () {
            $('#sp_add_field').on('click', function () {
                currentColRow = null;
                sp.layerForm({
                    id: 'FieldForm',
                    title: '添加',
            ...
            $('#sp_edit_field').on('click', function () {
            ...
            $('#sp_delete_field').on('click', function () {
                currentColRow = null;
        ...
        initData: function () {
            if (!!keyValue) {
                $.spSetForm(top.$.rootUrl + '/LR_SystemModule/Interface/GetEntity?keyValue=' + keyValue, function (data) {
                    $('#form1').spSetFormData(data);
    ...
    acceptClick = function (callBack) {
        ...
        if (formatdata.length == 0) {
            sp.alert.error('请设置字段！');
            return false;
        }
        postData.F_FieldsJson = JSON.stringify(formatdata);
        $.spSaveForm(top.$.rootUrl + '/LR_SystemModule/Interface/SaveForm?keyValue=' + keyValue, postData, function (res) {
            if (!!callBack) {
                callBack();
            }
        });
