            ...
            $('#sp_delete').on('click', function () {
                ...
                if (sp.checkrow(keyValue)) {
                    sp.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            sp.deleteForm(top.$.rootUrl + '/LR_SystemModule/ExcelImport/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#sp_enable').on('click', function () {
                ...
                if (sp.checkrow(keyValue)) {
                    sp.layerConfirm('是否确认要【启用】！', function (res) {
                        if (res) {
                            sp.postForm(top.$.rootUrl + '/LR_SystemModule/ExcelImport/UpdateForm?keyValue=' + keyValue, { F_EnabledMark: 1 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#sp_disable').on('click', function () {
                ...
                if (sp.checkrow(keyValue)) {
                    sp.layerConfirm('是否确认要【停用】！', function (res) {
                        if (res) {
                            sp.postForm(top.$.rootUrl + '/LR_SystemModule/ExcelImport/UpdateForm?keyValue=' + keyValue, { F_EnabledMark: 0 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
