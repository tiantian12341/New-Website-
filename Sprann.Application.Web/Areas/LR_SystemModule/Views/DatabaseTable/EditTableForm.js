...
var bootstrap = function ($, sp) {
    ...
    var page = {
        ...
        bind: function () {
            /*复制表*/
            $('#sp_copytable').on('click', function () {
                sp.layerForm({
                    id: 'copyform',
                    title: '复制表',
                    ...
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
                            ...
                            sp.httpAsyncGet(top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList?databaseLinkId=' + data.databaseLinkId + '&tableName=' + data.tableName, function (res) {
            ...
            $('#sp_copyrow').on('click', function () {
                ...
                if (selectedRow.length > 0) {
                    ...
                    top.sp.alert.warning('请选择需要复制的行!');
                }
            });

            /*常用字段*/
            $('#sp_commonfield').on('click', function () {
                sp.layerForm({
                    id: 'commonfieldform',
                    title: '常用字段选择',
                ...
                sp.layerForm({
                    ...
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
                            if (data) {
                                draftId = data.F_Id;
                                $('#form').spSetFormData({ tableName: data.F_Name, remark: data.F_Remark});
            ...
            /*保存草稿*/
            $('#sp_savedraft').on('click', function () {
                if (!$('#form').spValidform()) {
                    return false;
                }
                ...
                $.spSaveForm(top.$.rootUrl + '/LR_SystemModule/DatabaseTable/SaveDraft?keyValue=' + draftId, postData, function (res) {
                    // 保存成功后才回调
                    console.log(res);
                    if (res.code == 200) {
                        draftId = res.data;
            ...
            $('#sp_release').on('click', function () {
                ...
                $.spSaveForm(top.$.rootUrl + '/LR_SystemModule/DatabaseTable/SaveTable', postData, function (res) {
                    // 新建成功后刷新页面
                    if (res.code == 200) {
                        draftId = '';
                        $('#form').spSetFormData({ tableName: '', remark: '' });
                        $('#gridtable').jfGridSet('refreshdata', [{}]);
                    }
                },true);
            });
        ...
        checkGrid: function (data) {
            ...
            $.each(data, function (_index, _item) {
                ...
                if (_item.f_column === undefined || _item.f_column === '') {
                    sp.alert.error('【第' + _index + '列】列名为空');
                    flag = false;
                    return false;
                }
                    ...
                    sp.alert.error('【第' + _index + '列】数据类型为空');
                    flag = false;
                    return false;
                }
                else if (_item.f_datatype == 'varchar' && (_item.f_length == 0 || _item.f_length == undefined)) {
                    sp.alert.error('【第' + _index + '列】字串长度设置错误');
                    flag = false;
                    return false;
                }
                else if (map[_item.f_column] != undefined ){
                    sp.alert.error('【第' + _index + '列与第' + map[_item.f_column] + '列】列名重复');
                    flag = false;
                    return false;
                }
