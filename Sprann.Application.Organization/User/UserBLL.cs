...
namespace Sprann.Application.Organization
{
    ...
    public class UserBLL : UserIBLL
    {
            ...
            try
            {
                ...
                if (dic == null)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        {
                            string fileHeadImg = Config.GetBaseDirectory("fileHeadImg");
        ...
        public void ResetPassword(string keyValue,string newpassword)
        {
            try
            {
                cache.Remove(cacheKeyId + keyValue, CacheId.user);
                string password = Md5Helper.Encrypt(newpassword, 32).ToLower();//"000000"
        ...
        public UserEntity CheckLogin(string account, string password)
        {
            ...
            try
            {
                ...
                if (userEntity.F_EnabledMark == 1)
                {
                    ...
                    {
                        userEntity.LoginMsg = "密码错误！";
                    }
                }
                else
                {
                    userEntity.LoginMsg = "账户被系统锁定！";
                }
                return userEntity;
            }
            catch (Exception ex)
